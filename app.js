...
(function () {
  ...
  const STATE = {
    ...
    roomPrefix: null,
    config: loadConfig(),
    lossless: false,   // image/video quality mode (false = compressed, true = original)
  };
  ...
  const animals = ['fox', 'bear', 'hawk', 'cat', 'wolf', 'deer', 'rabbit', 'whale', 'shark', 'owl'];
  ...
  function openImage(src) {
    ...
    if (w) {
      w.document.write(
        `<title>Image Preview</title><style>body{margin:0;background:#000;display:flex;align-items:center;justify-content:center;min-height:100vh}img{max-width:100%;max-height:100vh}</style><img src="${src}">`,
      );
    } else {
      toast('Cannot open preview — allow popups and try again');
    }
  }
  ...
  function enterRoom(role, roomId) {
    ...
    appendMessage({ type: 'system', text: 'Connecting to signaling server…' });
    ...
    peer.on('open', (id) => {
      ...
      if (role === 'host') {
        appendMessage({ type: 'system', text: 'Room created. Waiting for others to join…' });
      }
      if (role === 'guest') {
    ...
    peer.on('error', (err) => {
      ...
      switch (err.type) {
        case 'unavailable-id':   msg = 'Room ID already in use, try another'; break;
        case 'network':         msg = 'Network error, please check your connection'; break;
        case 'server-error':    msg = 'Signaling server error'; break;
        case 'peer-unavailable':msg = 'Target device not found, check the room ID'; break;
        case 'ssl-unavailable': msg = 'SSL not available'; break;
      }
      toast(msg);
    ...
    peer.on('disconnected', () => {
      toast('Disconnected from signaling server, reconnecting…');
  ...
  async function sendImages(files) {
    ...
    for (const file of list) {
      try {
        ...
        console.error(e);
        toast('Image processing failed: ' + (e.message || e));
      }
    }
  }
  ...
  const _binChunks = new Map(); // id -> Uint8Array[]
  ...
  async function sendVideos(files) {
    ...
    if (!list.length) {
      toast('No video files found');
      return;
    }
    const n = list.length;
    toast(`Sending ${n} video${n > 1 ? 's' : ''}…`);
    for (const file of list) {
      try {
        ...
        console.error(e);
        toast('Video send failed: ' + (e.message || e));
      }
    }
  }
  ...
  joinBtn.addEventListener('click', () => {
    const id = prompt('Enter a 6-character room ID:');
    if (!id) return;
    const clean = id.trim().toUpperCase();
    if (!/^[A-Z0-9]{4,8}$/.test(clean)) { toast('Invalid room ID format'); return; }
    STATE.deviceId = genDeviceId();
    enterRoom('guest', clean);
  });

  leaveBtn.addEventListener('click', () => {
    if (confirm('Leave current room? All connections will be lost.')) leaveRoom();
  });
  ...
  copyUrlBtn.addEventListener('click', async () => {
    ...
    try {
      ...
      toast('Link copied — share it to join');
    } catch (_) {
      const ta = document.createElement('textarea');
      ta.value = url; document.body.appendChild(ta); ta.select();
      try { document.execCommand('copy'); toast('Copied'); }
      catch (e) { toast('Copy failed. URL: ' + url); }
  ...
  // Restore lossless preference from config
